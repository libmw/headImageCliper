package libs
{
	...
	public class Preview extends Sprite{
		 ...
		 * 创建一个预览图片框框
		 * */
		private function _buildPreview(size:int):void{
			...
			text.appendText(size + 'x' + size);
			this.addChild(text);
			
			
			StaticLib.console('log', 'imagex:' + image.x + 'imagey:' + image.y);
			
			_currentTop += size + 35; //间隔为5
		...
		public function render(bitmapData:BitmapData):void{
			...
			for(var i:int = 0; i < sizeList.length; i++){
				...
				var size:int = sizeList[i];
				
				var imageWidth:int = size;
				var imageHeight:int = size;
